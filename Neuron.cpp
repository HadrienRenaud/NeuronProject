...
Neuron::Neuron():   m_input(0),
                    ...
                    m_layer(0),
                    m_indexInLayer(0),
                    m_gradient(0){}//constructeur par défaut inutile
...
void Neuron::setTransfert(double (*trsf)(double)){m_trsf = trsf;}
transfert Neuron::getTransfert()const{return m_trsf;}
Layer* Neuron::getLayer()const{return m_layer;}
...
bool Neuron::initNeuron(double input){//on autorise de regler l'input si et seulement si il s'agit d'un neurone de la première couche
    if(getLayer()->getNetwork()->getFirstLayer()==getLayer()||input==0){
        m_input = input;
        if(input!=input){cout << "Neuron : initNeuron : if_fisrtlayer : input : "<< input << endl;}
        m_output = m_trsf(m_input);
        return true;
    }
    else{//si echec
...
inline double sigmo(double input){
    ...
    if (result!=result){cout << "sigmo probleme avec input = " << input << endl;}
    return result;
}
